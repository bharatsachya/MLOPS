...
df = ingest_data.get_data("E:\Regression\ols-regression-challenge-data\data\cancer_reg.csv")

constant_columns = find_constant_columns(df)
print("Columns that contains a single values: ", constant_columns)
...
df = drop_and_fill(df)
print(df.shape)
df.to_csv("ols-regression-challenge-data/cancer_reg_processed1.csv", index = False)